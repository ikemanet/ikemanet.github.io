---
layout: page
comments: true
title: 小驴拉磨 APP 官方网站 - 养成好习惯
type: ikema
featured_position: center
tags:
- 效率
- app
- uniloop
- 小驴拉磨
- 习惯养成
- 清单管理
- 小驴拉磨下载
- 官方下载
- 问题解答
- 拖延症
- 自律
---

查看其他语言：**[English](/uniloop/en/)**

<p align="center"><img src="/assets/img/uniloop/uniloop.jpg" style="width: 100%" /></p>

简单易用的日常事务管理工具，让你在循环中养成好习惯。非常感谢你的支持！这个页面主要用于联系作者，分享意见建议，以及获取最新版本更新说明。

## 下载

小驴拉磨（英文名：Uniloop）已在苹果 App Store 和谷歌 Google Play 上架。

最新版本：0.8.9 (46)

**苹果 iOS 版本：** [前往 App Store 下载](https://apps.apple.com/app/uniloop-habit-management/id1556095546)

**安卓 Android 版本：** [前往 Google Play 下载](https://play.google.com/store/apps/details?id=com.mzysstudio.uniloop)

使用安卓手机的国内的朋友，如果你在用手机品牌的应用商店暂未收录这个 APP，请点击下面的链接下载：

[最新安卓版本 0.8.9 (46)](https://github.com/ikemanet/uniloop/releases/download/apk0.8.9(46)/uniloop-0.8.9-46.apk)

<p align="center"><img src="/assets/img/uniloop/uniloop_zh_screenshots.jpg" style="width: 100%" /></p>

## 意见建议反馈

- 微信公众号：搜索``老马同学的相册`` (ID: ``foreverike``)，在公众号内可以给我留言
- 豆瓣小组：搜索``小驴拉磨``参与APP讨论
- Instagram ID：[_foreverike](https://instagram.com/_foreverike)

我会尽可能第一时间回复你，但由于工作关系，可能会延后回复，相信我，我会认真读完每一条留言。

## 隐私政策

[请点这里查看隐私政策](/uniloop/zh/privacy-policy)

## 版本更新说明

**0.8.9 (46)**
- 在循环的统计页面增加了日历，可让你全面了解循环的历史完成情况。
- 新功能：可让你设置循环具体的开始时间。
- 新功能：现在你可以把完成情况分享给你的朋友。

**0.6.3 (36)**
- 推出了全新的新增Loop的界面，更容易理解的选项和一个Loop预览画面
- 修改了完成过期Loop的逻辑，现在在完成后不会再有等待时间
- 你可以在贡献者页面检查是否有新版本
- 一些性能提升和错误修复